...
// This program reads the shared memory segment and increments the value by 1 after locking the semaphore
// It then releases the semaphore
// It is different from 49c1.c as it locks the semaphore twice because it is accessing two resources that are protected by the semaphore
// The resources are two shared memory segments
// The semaphore is a counting semaphore with value 2
// We creted two shared memory segments beacasue we need two resources to be protected by the semaphore
